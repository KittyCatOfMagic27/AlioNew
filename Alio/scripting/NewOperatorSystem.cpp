...
int main(){
    list<OpSpec> oplist;
    InitOperators(oplist);

    string sym = "+";
    VALUE x = {"69", INT};
    VALUE y = {"420", UINT};

    OpSpec* op = opMatcher(sym, x, y, oplist);
    if(op!=nullptr) op->print();
    else {cerr << "ERROR: invalid operator of " << sym << " with arguments " << x.value << " and " << y.value << "\n"; exit(-1);}
    cout << "This expression would evaluate as the type " << outTypeOfExpression(op, x, y) << "\n";
    return 0; 
}