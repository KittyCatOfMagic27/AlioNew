...
namespace lex{
  ...
  class LEXER{
    ...
    void parse(vector<Token> tks){
      ...
      unordered_map<string, string> ADDITIONAL_VAR_INFO;
      for(int i = 0; i < tks.size(); i++){
        ...
        if(tk.type==PROC){
          ...
          if (tks[++i].type == OPARA){
            ...
            ADDITIONAL_VAR_INFO[result.value] = tks[i-2].value;
            Tokens.push_back(result);
          }
          else{
            result.value = tks[i].value;
          ...
          if(ADDITIONAL_VAR_INFO.find(tk.value) != ADDITIONAL_VAR_INFO.end()){
            result.value = ADDITIONAL_VAR_INFO[tk.value];
            result.type  = VALUE;
            Tokens.push_back(result);
          }
        }
