...
namespace intr{
  inline bool DEF_ASSIGN(lex::Token tk){
    return (tk.type>=lex::UINT&&tk.type<=lex::AMPERSAND) ||tk.type==lex::SYSCALL || tk.type==lex::OPERATOR;
  }
  ...
  class INTERMEDIATE{
    ...
    INTERMEDIATE(vector <lex::Token> &_Tokens, list<ops::OpSpec> *_oplist): oplist(_oplist){
      Tokens = _Tokens;
    }
    ...
    void tks_to_inter(){
      if(options::STAGETEXT)printf("Generate intermediate representation...\n");
      lex::Token tk1;
      lex::Token tk2;
      string prev;
      ...
      for(int i = 0; i<Tokens.size(); i++){
        ...
        switch(tk.type){
          ...
          case lex::OPERATOR:{
            ...
            tk2 = Tokens[++i];
            INTER.push_back("op");
            INTER.push_back(ptrToHex(tk.ptr));
            setup_op(tk1, tk2); // only supports OP2
          }
          break;
          case lex::ADD:
          tk1 = Tokens[++i];
      ...
      if(options::STAGETEXT)printf("Done generating intermediate representation!\n");
    }
