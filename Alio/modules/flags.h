...
namespace kf{
  ...
  class FlagProxy{
    public:
      ...
      void MakeFlagRequired(std::string flag){
        RQFLAGS.push_back(flag);
      }
      ...
      void Parse(int argc, char** argv){
        ...
        for(int i = 0; i < RQFLAGS.size(); i++){
          bool foundindex = false;
          for(int j = 0; j < usedFlags.size(); j++){
            if (usedFlags[j] == RQFLAGS[i]){foundindex = true; break;}
          }
          if(!foundindex) {
            std::cerr << "\033[1;31mFLAG '"<< RQFLAGS[i] <<"' IS REQUIRED AND NOT FOUND!\033[0m\n";
            PrintHelpDesc();
            assert(false);
          }
        }
